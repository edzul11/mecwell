...
def get_expenses(supabase: Client):
    response = supabase.table("company_expenses").select("*, sites(name)").order("created_at", desc=True).execute()
    return response.data

def get_expense(expense_id: str, supabase: Client):
    response = supabase.table("company_expenses").select("*, sites(name)").eq("id", expense_id).execute()
    if not response.data:
        raise HTTPException(status_code=404, detail="Expense not found")
    return response.data[0]

def create_expense(expense: CompanyExpenseCreate, supabase: Client):
    response = supabase.table("company_expenses").insert(expense.dict(exclude_none=True)).execute()
    if not response.data:
...
def update_expense(expense_id: str, expense: CompanyExpenseUpdate, supabase: Client):
    ...
    response = supabase.table("company_expenses").update(update_data).eq("id", expense_id).execute()
    if not response.data:
        raise HTTPException(status_code=404, detail="Expense not found")
    return response.data[0]

def delete_expense(expense_id: str, supabase: Client):
    response = supabase.table("company_expenses").delete().eq("id", expense_id).execute()
    return response.data
